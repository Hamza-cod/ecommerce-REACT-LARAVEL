...
const selectedProduct = allProducts?.find((item) => item.id == id)
    ...
    if(selectedProduct){
        let { image, price, name: title } =  selectedProduct;
        return(
  ...
  <div className="px-4 pt-8">
    ...
    <div className="mt-8 space-y-3 rounded-lg border bg-white px-2 py-4 sm:px-6">
      <div className="flex flex-col rounded-lg bg-white sm:flex-row">
        <img className="m-2 h-24 w-28 rounded-md border object-cover object-center" src={import.meta.env.VITE_BANCKEND_URL + "/storage/" + image} alt={title} />
        <div className="flex w-full flex-col px-4 py-4">
          <span className="font-semibold">{title}</span>
          <span className="float-right text-gray-400">{selectedProduct?.category?.name}</span>
  ...
  <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
    ...
    <div className="">
      ...
      <div className="relative">
        <input type="text" id="email" name="email" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder="06/07 0000...." />
        <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div className="flex flex-col sm:flex-row">
        <div className="relative flex-shrink-0 sm:w-7/12">
          <input type="text" id="billing-address" name="billing-address" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500" placeholder=" Address" />
          <div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
            <img className="h-4 w-4 object-contain" src="https://upload.wikimedia.org/wikipedia/commons/2/2c/Flag_of_Morocco.svg" alt="" />
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <p className="text-2xl font-semibold text-gray-900">${price+8}</p>
      </div>
    </div>
    <button className="mt-4 mb-8 w-full rounded-md bg-gray-900 px-6 py-3 font-medium text-white">Order Now</button>
  </div>
        </div>)
